...
ConfinementForce::ConfinementForce(Cloud *myCloud, double confineConst) : Force(myCloud), confine(-confineConst) {}
...
void ConfinementForce::writeForce(fitsfile *file, int *error)
{
	//move to primary HDU:
	if(!*error)
...
void ConfinementForce::readForce(fitsfile *file, int *error)
{
	//move to primary HDU:
	if(!*error)
		//file, # indicating primary HDU, HDU type, error
