...
// check if force is in compatible with other force
...
int main (int argc, char * const argv[]) 
{
	...
	double thermRed1 = thermRed;		//default inner reduction factor (-L)	//TODO: inner or outer?
	...
	for(int i = 0; i < argc; i++)
	{
		switch(argv[i][1])
		{
			...
			case 'h': // display "h"elp:
				...
				exit(1);
				break;
			case 'L': // perform laser heating experiment:
			...
			case 'T': // set "T"emperature reduction factor:
				...
				break;
			case 'v': // set time "v"arying thermal force 
...
-----------------------------------------------------------------------------*/
	...
	if (!continueFileIndex)
	{
		...
		{	
			...
			//	prevents fits from shitting itself when creating binary tables
...
-----------------------------------------------------------------------------*/
	...
	// Initalize forces from fits file if this is continuing a file otherwise write force parameters to the fitsfile.
	if(continueFileIndex) 
	{
